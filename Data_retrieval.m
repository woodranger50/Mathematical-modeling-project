% function out=Data_retrieval(T_water)
...
%Mass water
m_water_initial=0.2;    %[kg] initial mass of water in beaker
...
Nu_L_horizontal=0.54*Ra_L^(1/4);
h_horizontal=Nu_L_horizontal*k_water_var/L;                 %[W/m^2*K]
Q_horizontal=h_horizontal*A_small_water*(T_film-T_surr); %J/s

Q_current=T_water*cp_water(T_water)*m_water_initial;
Q_convection=Q_horizontal+Q_vertical;       %J/s
T_new=(Q_current-Q_convection)/(cp_water(T_water)*m_water_initial);     %Needs time interval and evaporation
%% Radiation
...
%% ODEtspan = [0 5];
tspan = [0 5];
m0 = 200;       %[kg]   Initial mass
T0 = 80;        %[°C]   Initial temperature
[t,y] = ode45(@(t,y) 2*t, tspan, m0, T0);
figure
hold on
plot(t,y,'-o')
hold off
